...
let input = try String(contentsOf: URL(fileURLWithPath: "../input/day06")).split(separator: "\n")
  .map {
    $0.split(separator: ":").map { $0.trimmingCharacters(in: .whitespaces) }[1].split(
      separator: " "
    ).map { Int($0)! }
  }
let inputPart2 = try String(contentsOf: URL(fileURLWithPath: "../input/day06_part2")).split(
  separator: "\n"
).map {
  $0.split(separator: ":").map { $0.trimmingCharacters(in: .whitespaces) }[1].split(separator: " ")
    .map { Int($0)! }
}
...
func highestCharge(allowedTime: Int, requiredDistance: Int) -> Int {
  stride(from: allowedTime, to: 0, by: -1).first(where: {
    $0 * (allowedTime - $0) > requiredDistance
  })!
}
