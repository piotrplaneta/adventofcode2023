...
let input = try String(contentsOf: URL(fileURLWithPath: "../input/day12")).split(separator: "\n").map { ($0.split(separator: " ")[0], $0.split(separator: " ")[1].split(separator: ",").map { Int($0)! }) }
  ...
  if calculatedWays.keys.contains(RowAndConditions(row: row, conditions: conditions)) {
    ...
    let calculated = switch (row, conditions) {
      ...
      case (_, _) where row.first == "#": {
        ...
        return if availableSpace < springsToPut || (springsToPut < row.count && row[row.index(row.startIndex, offsetBy: springsToPut)] == "#") {
          0
        }
        else {
      ...
      case (_, _) where row.first == "?": {
        waysOfPuttingSprings(("#" + row.dropFirst(), conditions)) + waysOfPuttingSprings((row.dropFirst(), conditions))
      }()
      case (_, _): 0
    }
...
let unfoldedInput = input.map { test in
  ...
  return (Substring.SubSequence([Substring.SubSequence](repeating: row, count: 5).joined(separator: "?")), Array([[Int]](repeating: conditions, count: 5).joined()))
}
