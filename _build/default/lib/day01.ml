...
let input_with_words_to_digits =
  input
  |> List.map ~f:(fun line ->
         line
...
let calibration_codes lines =
  let lines_digits =
    ...
    |> List.map ~f:(List.filter ~f:Char.is_digit)
  in
  lines_digits
  |> List.map ~f:(fun line_digits ->
         String.of_char_list
           [ List.hd_exn line_digits; List.last_exn line_digits ]
         |> Int.of_string)

let part_one = input |> calibration_codes |> List.reduce_exn ~f:( + )

let part_two =
  input_with_words_to_digits |> calibration_codes |> List.reduce_exn ~f:( + )
